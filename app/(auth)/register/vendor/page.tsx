...
export default function VendorRegistration() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success("Registration submitted! Admin review is pending.");
        setTimeout(() => {
      ...
      <div className="w-full max-w-2xl font-sans">
        ...
        <div className="flex items-center justify-center gap-2 mb-8 text-xs font-semibold text-navy-850">
          ...
          <div className="flex items-center gap-1.5 text-navy-950 font-bold">
            <span className="w-5 h-5 rounded-full bg-navy-900 text-gold-500 flex items-center justify-center text-[10px]">2</span>
            <span>Service Details</span>
          </div>
          <ChevronRight className="w-3.5 h-3.5 text-navy-200" />
          <div className="flex items-center gap-1.5 text-navy-700">
            <span className="w-5 h-5 rounded-full bg-navy-100 text-navy-700 flex items-center justify-center text-[10px]">3</span>
            <span>Approval</span>
          </div>
        </div>
        ...
        <Card className="w-full border-[#E8E0D0] bg-white rounded-card shadow-sm">
          <CardHeader className="text-center pb-2">
            ...
            <CardDescription className="text-xs text-navy-750 font-sans mt-1">Join Aadana Tharakar to offer your professional services</CardDescription>
          </CardHeader>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="bg-navy-50 p-4 rounded-btn border border-navy-100 flex items-start gap-3">
                ...
                <p className="text-xs text-navy-800 leading-normal font-medium">
                  Your vendor application will be submitted for administrative verification. You will receive an email confirmation once approved.
                </p>
              </div>
